...
const Orders = ({}) => {
    ...
    const getData = () => {
        setloading(true)
        setorderData([])
        dispatch(Order_SET({customer_id:1},"mpreviousOrderDetailsByCustomerId",loginData.data.token))
        setloading(false)
    }
            ...
            <LoadingModal loading={Order_Data.loading} setloading={setloading} />
                ...
                <TouchableOpacity onPress={() => {navigation.push('OrderDetails',{id:i.id})}}>
                    <CartBox>
                        ...
                        <Text style={globalStyles.order_title2}>{/* 06 Dec 2022 at 10:40AM */}{i.created_at}</Text>
                    </CartBox>
                    <Text></Text>
